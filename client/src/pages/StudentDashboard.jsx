...
export default function StudentDashboard() {
  ...
  const filteredComplaints = useMemo(() => {
    return complaints
      .filter((c) => {
        ...
        if (priorityFilter && c.priority !== priorityFilter) {
          return false;
        }
  ...
  const handleExecuteDelete = async () => {
    ...
    try {
      await api.delete(
        `/complaints/${deleteModal.complaintId}`
      );

      setComplaints((prev) =>
        prev.filter(
          (c) =>
            c.complaintId !== deleteModal.complaintId &&
            c._id !== deleteModal.complaintId
        )
      );

      setActionNotice(
        `Complaint ${deleteModal.complaintId} was successfully deleted.`
      );
            ...
            <div className="flex items-start space-x-2.5">
              ...
              <div className="text-xs sm:text-sm">
                <strong className="font-bold block">
                  Restricted Administrative Route
                </strong>
          ...
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4 sm:p-5 shadow-2xs animate-in fade-in flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-start space-x-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                {(recentResolvedComplaint.resolutionDetails?.summary ||
                  recentResolvedComplaint.resolutionDetails?.actionTaken ||
                  recentResolvedComplaint.resolutionDetails?.message) && (
              ...
              <div className="md:hidden divide-y divide-slate-100">
                ...
                {filteredComplaints.map((c) => (
                  <div
                    ...
                    className="p-3.5 space-y-2.5 hover:bg-slate-50/70"
                  >
