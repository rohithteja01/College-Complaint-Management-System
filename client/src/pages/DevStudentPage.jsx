...
import { Loader2, User, CheckCircle2, AlertCircle, RefreshCw, ArrowRight } from 'lucide-react';
import { useAuth } from '../context/AuthContext';

export default function DevStudentPage() {
  const { devLogin } = useAuth();
  const navigate = useNavigate();
  const [status, setStatus] = useState('Initializing Development Student Session...');
  ...
  const initSession = async () => {
    setLoading(true);
    setError(null);
    setStatus('Connecting and activating Test Student profile...');

    try {
      await devLogin('student');
      setStatus('Development Student session active! Redirecting to Dashboard...');
      setTimeout(() => {
        navigate('/student/dashboard', { replace: true });
      }, 400);
    } catch (err) {
      console.error('Dev student initialization failed:', err);
      setError(err.response?.data?.message || err.message || 'Failed to initialize development student session.');
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white p-8 rounded-2xl border border-slate-200 text-center shadow-2xs space-y-4 animate-in fade-in">
        <div className="w-12 h-12 rounded-xl bg-emerald-100 text-emerald-700 mx-auto flex items-center justify-center">
          {error ? <AlertCircle className="w-6 h-6 text-rose-600" /> : <User className="w-6 h-6" />}
        </div>
        
        <h2 className="text-xl font-bold text-slate-900">Dev Mode: Student Access</h2>
