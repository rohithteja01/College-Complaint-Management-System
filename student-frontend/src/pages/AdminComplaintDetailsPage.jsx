...
export default function AdminComplaintDetailsPage() {
  ...
  const fetchComplaint = async () => {
    ...
    try {
      ...
      const deptId = data.assignedDepartment?._id || data.assignedDepartment || '';
      setSelectedDept(deptId);

      const staffId = data.assignedStaff?._id || data.assignedStaff || '';
  ...
  const fetchStaffForDept = async (deptId) => {
    ...
    try {
      const res = await api.get(`/admin/departments/${deptId}/staff?activeOnly=true`);
